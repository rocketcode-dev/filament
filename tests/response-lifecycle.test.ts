...
class ControlledServerResponse {
  ...
  readonly heads: Array<{ status: number, headers: string[] }> = [];
  readonly writeCallbacks: WriteCallback[] = [];
  readonly endCallbacks: EndCallback[] = [];
  endCalls = 0;

  writeHead(status: number, headers: string[]): this {
    this.heads.push({ status, headers });
    return this;
  }

  write(data: Buffer, callback: WriteCallback): boolean {
  ...
  finishEnd(): void {
    ...
    if (!callback) throw new Error('No pending end');
    callback();
  }
}

function response(streaming: boolean): {
  native: ControlledServerResponse,
  res: Response,
} {
  const native = new ControlledServerResponse();
  const res = new Response(native as unknown as ServerResponse);
