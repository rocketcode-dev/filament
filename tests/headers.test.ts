...
describe('Headers', () => {
  test('constructs headers from records, pairs, and lists', () => {
    const headers = new Headers(
      'response',
      { 'content-type': 'text/plain', 'x-tag': ['one', 'two'] },
      ['set-cookie', ['a=1', 'b=2']],
      [
        ['x-trace', 'first'],
        ['x-trace', 'second'],
  ...
  test('sets and removes individual and bulk headers fluently', () => {
    ...
    assert.equal(headers.addMany(
      ['X-Trace', 'trace-1'],
      { 'X-Trace': 'trace-2', 'Content-Type': 'text/plain' },
    ), headers);
    assert.deepEqual(headers.get('X-Trace'), ['trace-1', 'trace-2']);

    assert.equal(headers.setMany(
      ['X-Trace', ['trace-3', 'trace-4']],
      { 'X-Tag': ['three', 'four'], 'Content-Type': 'application/json' },
    ), headers);
