...
function compileExample(example: Example): Promise<Compilation> {
...
async function waitForServer(
  ...
  while (Date.now() < deadline) {
    ...
    const listening = await new Promise<boolean>(resolveConnection => {
...
async function withExample(
  ...
  run: (request: (
    path: string,
    init?: RequestInit,
  ) => Promise<ExampleResponse>, baseUrl: string) => Promise<void>,
  serverArgs: string[] = [],
): Promise<void> {
  ...
  if (!existsSync(compilation.outputFile)) {
    throw new Error(
      `TypeScript did not emit ${example.source}:\n${compilation.diagnostics}`,
    );
  }
...
async function runExampleClient(
  client: Example,
  baseUrl: string,
): Promise<any> {
  ...
  if (!existsSync(compilation.outputFile)) {
    throw new Error(
      `TypeScript did not emit ${client.source}:\n${compilation.diagnostics}`,
    );
  }
  const result = await execFileAsync(process.execPath, [
    compilation.outputFile,
...
function testCompilation(example: Example): void {
  TestBattery.test('should compile', async battery => {
    ...
    battery.test(
      result.diagnostics
        ...
        : 'should compile without diagnostics',
    ).value(result.status).value(0).equal;
  });
}
