...
suite('Observability', () => {
    ...
    test('captures streaming chunks before commit and then stops', async () => {
        ...
        app.onFinalize(req => {
            snapshots.push(JSON.parse(JSON.stringify(req.context.application?.observed)));
        });
        ...
        try {
            ...
            assert.deepEqual(snapshots[0].trace?.map(entry => [entry.type, entry.endStatus]), [
                ['system', 'open'],
                ['route', 'committed'],
            ]);
        }
    ...
    test('collects the maximum, narrows by fixed status, and traces policies', async () => {
        ...
        app.get('/dynamic-opt-out', { label: 'opt-out' }, async function optOut(req, res) {
            var _a;
            (_a = req.context).application ?? (_a.application = {});
        ...
        app.onFinalize((req, res) => {
            ...
            if (observed) {
                snapshots.push(JSON.parse(JSON.stringify(observed)));
                committedAtFinalize.push(res.committed);
            }
        });
        const port = await app.listen(0);
        try {
            const createdResponse = await fetch(`http://127.0.0.1:${port}/created?source=test`, {
                method: 'POST',
                headers: {
            ...
            assert.match(created.requestId, /^\d{8}-\d{6}-\d{4}-[0-9a-z]{5}$/);
            ...
            assert.deepEqual(created.responseInfo?.headers?.['X-Transformed'], ['yes']);
            assert.deepEqual(created.trace?.map(entry => [entry.type, entry.name, entry.endStatus]), [
                ['system', 'request', 'open'],
                ['middleware', 'observePolicyBoundary', 'open'],
                ['route', 'createWidget', 'closed'],
                ['transformer', 'replaceCreatedBody', 'closed'],
            ]);
            assert.deepEqual(created.trace?.map(entry => entry.endTime), [...created.trace].map(entry => entry.endTime).sort((a, b) => a - b));
            ...
            assert.equal(failure.responseInfo?.body, JSON.stringify({ error: 'Internal Server Error' }));
            assert.equal(failure.requestInfo?.headers?.['X-Captured']?.[0], 'yes');
            assert.equal(failure.requestInfo?.origin, '198.51.100.20');
            assert.deepEqual(failure.trace?.map(entry => entry.type), ['system', 'middleware', 'route', 'error']);
